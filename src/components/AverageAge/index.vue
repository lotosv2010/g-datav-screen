  ...
  <div class="average-age">
    <div class="title-wrapper">
      <div class="average-age-left">
        <div class="title">华为手机用户年龄分布&平均年龄</div>
  ...
  setup(ctx) {
    ...
    const updateChart = () => {
      ...
      const data = source.reduce(
        (pre, item) => pre.concat(item.value),
        ["指标"]
      );
      const max = source.reduce((pre, item) => pre + item.value, 0);
      const color = source.map((item) => item.color);
      const axis = source.reduce(
        (pre, item) => pre.concat(item.axis),
        ["指标"]
      );
      options.value = {
        color,
